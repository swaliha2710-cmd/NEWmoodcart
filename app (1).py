    ...
    if st.session_state["df"] is None:
        ...
        if st.session_state.get("clf_results") is not None:
            ...
            if conf_mats and best_name in conf_mats and le is not None:
                ...
                fig = px.imshow(cm, x=list(le.classes_), y=list(le.classes_),
    ...
    if st.session_state["df"] is None:
        ...
        if seg_col:
            ...
            if "Mood" in df.columns:
                ...
                melted5 = melted5.rename(columns={"variable": "Mood", "value": "%"})
                melted5["label"] = melted5["%"].round(0).astype(int).astype(str) + "%"
                fig5 = px.bar(melted5,
                              x=seg_col, y="%", color="Mood",
