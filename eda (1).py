...
def run_eda(df: pd.DataFrame):
    ...
    with tabs[0]:
        ...
        counts["Interest"] = counts["Interest"].astype(str)
        counts["Pct"] = (counts["Count"] / counts["Count"].sum() * 100).round(1)
        present_int = counts["Interest"].unique().tolist()
        cmap_ov = {k: v for k, v in INTEREST_COLORS.items() if k in present_int}

        col_a, col_b = st.columns(2)
        with col_a:
            fig = px.bar(counts, x="Interest", y="Count",
                         color="Interest", text=counts["Pct"].astype(str) + "%",
                         color_discrete_map=cmap_ov if cmap_ov else None)
        ...
        with col_b:
            fig2 = px.pie(counts, names="Interest", values="Count",
                          hole=0.5,
                          color="Interest", color_discrete_map=cmap_ov if cmap_ov else None)
    ...
    with tabs[1]:
        ...
        if "Age" in df.columns and "Interest_in_MoodCart" in df.columns:
            ...
            melted.columns = ["Age", "Interest", "%"]
            melted["Interest"] = melted["Interest"].astype(str)
            melted["%"] = pd.to_numeric(melted["%"], errors="coerce").fillna(0)
            melted["label"] = melted["%"].round(1).astype(str) + "%"
            present_interests = melted["Interest"].unique().tolist()
            cmap = {k: v for k, v in INTEREST_COLORS.items() if k in present_interests}
            fig = px.bar(melted,
                         x="Age", y="%", color="Interest",
        ...
        if "Income" in df.columns and "Interest_in_MoodCart" in df.columns:
            ...
            melted2.columns = ["Income", "Interest", "%"]
            melted2["Interest"] = melted2["Interest"].astype(str)
            melted2["%"] = pd.to_numeric(melted2["%"], errors="coerce").fillna(0)
            melted2["label"] = melted2["%"].round(1).astype(str) + "%"
            present_interests2 = melted2["Interest"].unique().tolist()
            cmap2 = {k: v for k, v in INTEREST_COLORS.items() if k in present_interests2}
            fig = px.bar(melted2,
                         x="Income", y="%", color="Interest",
                         barmode="stack", text="label",
                         color_discrete_map=cmap2 if cmap2 else None)
